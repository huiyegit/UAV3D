    ...
    def position_embeding(self, img_feats, img_metas, masks=None):
        ...
        if 'img_aug_matrix' in img_metas[0]:
            ...
            for img_meta in img_metas:
                post_rot = [ ]
                post_tran = []
                for i in range(len(img_meta['img_aug_matrix'])):
                    img_rot = img_meta['img_aug_matrix'][i][:3, :3]
                    post_rot.append(np.linalg.inv(img_rot))
                    img_tran = img_meta['img_aug_matrix'][i][:3, 3]
                    post_tran.append(img_tran)

                post_rots.append(np.asarray(post_rot))
                post_trans.append(np.asarray(post_tran))
            post_rots = np.asarray(post_rots)
            post_rots = coords.new_tensor(post_rots)
            post_trans = np.asarray(post_trans)
            post_trans = coords.new_tensor(post_trans)

            coords = coords - post_trans.view(B, N, 1, 1, 1, 3)
            coords = post_rots.view(B, N, 1, 1, 1, 3, 3).matmul(coords.unsqueeze(-1))
        ...
        for img_meta in img_metas:
            ...
            for i in range(len(img_meta['intrinsics'])):
                img2cam.append(np.linalg.inv(img_meta['intrinsics'][i]))
            img2cams.append(np.asarray(img2cam))
        img2cams = np.asarray(img2cams)
        img2cams = coords.new_tensor(img2cams)
        img2cams = img2cams.view(B, N, 1, 1, 1, 4, 4).repeat(1, 1, W, H, D, 1, 1)
        ...
        for img_meta in img_metas:
            cam2lidar_rot = [ ]
            cam2lidar_tran = [ ]
            for i in range(len(img_meta['cam2lidar'])):
                cam2lidar_rot.append(img_meta['cam2lidar'][i][:3, :3])
                cam2lidar_tran.append(img_meta['cam2lidar'][i][:3, 3])
            cam2lidar_rots.append(np.asarray(cam2lidar_rot))
            cam2lidar_trans.append(np.asarray(cam2lidar_tran))

        cam2lidar_rots = np.asarray(cam2lidar_rots)
        cam2lidar_rots = coords.new_tensor(cam2lidar_rots)
        cam2lidar_trans = np.asarray(cam2lidar_trans)
        cam2lidar_trans = coords.new_tensor(cam2lidar_trans)

        coords3d = cam2lidar_rots.view(B, N, 1, 1, 1, 3, 3).matmul(coords3d.unsqueeze(-1)).squeeze(-1)
        coords3d += cam2lidar_trans.view(B, N, 1, 1, 1, 3)

        if 'lidar_aug_matrix' in img_metas[0]:
            extra_rots = []
            extra_trans = []
            for img_meta in img_metas:
                extra_rot = img_meta['lidar_aug_matrix'][:3, :3]
                extra_tran = img_meta['lidar_aug_matrix'][:3, 3]
                extra_rots.append(np.asarray(extra_rot))
                extra_trans.append(np.asarray(extra_tran))
            extra_rots = np.asarray(extra_rots)
            extra_rots = coords3d.new_tensor(extra_rots)
            extra_trans = np.asarray(extra_trans)
            extra_trans = coords3d.new_tensor(extra_trans)

            coords3d = extra_rots.view(B, 1, 1, 1, 1, 3, 3).repeat(1, N, 1, 1, 1, 1, 1)\
                .matmul(coords3d.unsqueeze(-1)).squeeze(-1)
            coords3d += extra_trans.view(B, 1, 1, 1, 1, 3).repeat(1, N, 1, 1, 1, 1)
