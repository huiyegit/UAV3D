...
class BEVFusion(Base3DFusionModel):
    ...
    def extract_camera_features(
        ...
        num_agent = 5
        # x_com = x_com[:,0,:].unsqueeze(dim=1)
        x_com = x_com.repeat(1, num_agent, 1, 1, 1)
        x_com = x_com.view(-1, C, H, W)
        
        BN, C, H, W = x[0].size()
        x_0 = x[0].view(B, N, C, H, W)
        x_0 = x_0[:,0:5,:]
        
        # x_0 = torch.mean(x_0, dim=1)
        # x_0 = x_0.unsqueeze(dim=1)
        x_0 = x_0.repeat(1, num_agent, 1, 1, 1)
        x_0 = x_0.reshape(-1, C, H, W)
        
        BN, C, H, W = x[2].size()
        x_2 = x[2].view(B, N, C, H, W)
        x_2 = x_2[:,0:5,:]
        # x_2 = torch.mean(x_2, dim=1)
        # x_2 = x_2.unsqueeze(dim=1)
        x_2 = x_2.repeat(1, num_agent, 1, 1, 1)
        x_2 = x_2.reshape(-1, C, H, W)
