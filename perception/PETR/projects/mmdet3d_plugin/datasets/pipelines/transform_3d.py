...
class GlobalRotScaleTransImage(object):
    ...
    def __init__(self, rot_range=[-0.3925, 0.3925],
                 ...
                 reverse_angle=False,
                 training=True,):
        self.resize_lim = scale_ratio_range
        self.rot_lim = rot_range
        self.trans_lim = translation_std
        self.is_train = training
    ...
    def __call__(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.is_train:
            ...
            translation = np.array([random.normal(0, self.trans_lim) for i in range(1)])
            rotation = np.eye(3)
            ...
            rot_cos = torch.cos(torch.tensor(theta))
            rot_sin = torch.sin(torch.tensor(theta))
            rot_mat = torch.tensor([[rot_cos, -rot_sin, 0],
                                    [rot_sin, rot_cos, 0],
                                    [0, 0, 1]])
            rotation = rotation @ rot_mat.numpy()
