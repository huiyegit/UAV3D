...
class BaseTransform(nn.Module):
    ...
    def get_geometry(
        ...
        **kwargs,
        ...
        points = camera2lidar_rots.view(B, N, 1, 1, 1, 3, 3).matmul(points).squeeze(-1)
        # points[:, :, :, :, :, 1:2] = points[:, :, :, :, :, 0:1]* (-1)
    ...
    def forward(
        ...
        **kwargs,
        ...
        temp = []
        geom_split = torch.split(geom, 1, dim = 1)
        x_split = torch.split(x, 1, dim = 1)
        for i in range(x.shape[1]):
            temp.append(self.bev_pool(geom_split[i], x_split[i]))
        # import pdb
        # pdb.set_trace()
        x = torch.stack(tuple(temp),dim=1)
