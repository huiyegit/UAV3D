...
from projects.splits import create_splits_scenes
...
def load_gt(nusc: NuScenes, eval_split: str, box_cls, verbose: bool = False) -> EvalBoxes:
    ...
    for sample_token in tqdm.tqdm(sample_tokens, leave=verbose):
        ...
        sd_record = nusc.get('sample_data', sample_rec['data']['CAMERA_BOTTOM_id_0'])
        pose_record = nusc.get('calibrated_sensor', sd_record['calibrated_sensor_token']).copy()
        pose_record['translation'][2] = 0

        for sample_annotation_token in sample_annotation_tokens:

            sample_annotation = nusc.get('sample_annotation', sample_annotation_token)

            center = sample_annotation['translation'].copy()
            center = np.array(center) - np.array(pose_record['translation'])
            flag = False
            ...
            if box_cls == DetectionBox:
                ...
                from projects.utils import category_to_tracking_name
